return {
  ...
  opts = {
    ...
    default = {
      ...
      dir_path = function()
        -- Function to search upward for .obsidian folder
        local function find_obsidian_root(path)
          local current_dir = vim.fn.fnamemodify(path, ":p:h")

          -- Keep searching upward until we hit the root
          while current_dir ~= "/" do
            local obsidian_dir = current_dir .. "/.obsidian"
        ...
        local vault_root = find_obsidian_root(current_file)
        print(vault_root .. "/Images")

        if vault_root then
          -- We're in an Obsidian vault, return absolute path to Images folder
          return vault_root .. "/Images"
        else
          -- Not in an Obsidian vault, use the old behavior
